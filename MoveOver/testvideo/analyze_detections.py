...
class AnalyzeDetections():
    def __init__(self, Paths):
        ...
        # TODO: are these manually made? (esp first 4)
        self.LANES_FROM = (1, 2) # The cars should move over from these lanes
        self.LANES_TO = (4, 5) #the target lanes for the cars from lanes_from (we want to move over to this lane)

        self.FAST_LANES = (1, 4) # Lanes before slow down
        self.SLOW_LANES = (2, 5) # Lanes after slowing down

        self.DIST_FRONT = 15 # Free space in front of the car (on the next lane) [m]
        self.DIST_BEHIND = 20 # Free space behind the car (on the next lane) [m]
        self.CAN_CHANGE_TIME = 9 # minimum time required to change the lane [s]
        self.SPEED_THRESHOLD = 0.8 # if speed_before * SPEED_THRESHOLD > speed_after - the vehicle slowed down

        self.MIN_TIME = 0.2 # Minimum lifetime of the object
        
         # read lane detections
        self.dfraw = pd.read_csv('./data/lanes_detections.csv', header=None)
        self.dfraw.columns = ['frame', 'lane', 'objectId', 'objectType', 'secMark', 
                        'xLeft', 'xRight', 'yTop', 'yBottom', 'lat', 'lon', 
                        'speed', 'heading', 'elevation'] 
        self.dfraw.speed = self.dfraw.speed.replace('None', np.nan).astype(float)

        if self.SKIP_FIRST is not None:
            self.dfraw = self.dfraw[self.dfraw.frame >= self.SKIP_FIRST*self.fps] # Start processing from 25:10

        self.dfraw.head()
        
        ###
        objectIds = self.getObjectIds(self.dfraw, self.MIN_TIME, self.fps)
        vector, vectorNorm, vectorPerp = self.getVectors(self.dfraw, self.LANES_FROM, objectIds)
    ...
    def getObjectIds(self, dfraw, min_time, fps):
        """
    ...
    def normalizeVector(self, point, vector):
        """
    ...
    def lineEquation(self, point, vector):
        """
    ...
    def getDistance(self, point, line):
        """
    ...
    def isBetween(self, point, line1, line2):
        """
    ...
    def isAnyBetween(self, df, line1, line2):
        """
        Arguments:
    ...
    def getVectors(self, dfraw, lanes_from, objectIds):
        """
    ...
    def checkFreeSpace(self, dfraw, vectorNorm, dist_front, dist_behind, lanes_from, lanes_to):
        """
        ...
        for i, frame in enumerate(frames):
            ...
            for idx, row in framedf[framedf.lane.isin(lanes_from)].iterrows():
                ...
                lane_front = self.lineEquation(point, vector)
                point = {'lon' : row.lon_behind, 'lat' : row.lat_behind}
                lane_behind = self.lineEquation(point, vectorNorm)
                if self.isAnyBetween(totestdf, lane_front, lane_behind):
    ...
    def lane_changed(self, df, laneObjects, lanes_from, lanes_to = None, never_on = None, verbose = False):   
        """
    ...
    def checkSlow(self, df, fast_lanes, slow_lanes):
